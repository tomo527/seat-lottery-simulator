...
- TOKYO-WAVE-16 is validated and ready for its authorized commit/push: exactly 台東区立旧東京音楽学校奏楽堂〔音楽ホール〕 (`tokyo-wave3-0339/standard`) was directly transcribed from the current-linked official printed map. It has **310 mapped / 310 official / rangeDiff 0 / representative**; the prior Tokyo inventory value **311** is corrected to current official **310**. No capacity fitting, invented IDs, duplicate suppression, geometric ownership, or configuration mixing was used.
...
Complete the authorized Wave16 commit/push and managed deployment/public-runtime verification only. Do not research or implement Wave17 without separate authorization. Reopen an individual Wave13 source only if an issuer-defined row/area key or an equivalently explicit current official numbered-group label becomes available; do not infer it from diagram geometry.
