...
- TOKYO-WAVE-15 source integration is staged for exactly よみうり大手町ホール and 八王子市南大沢文化会館〔主ホール〕. Their source, Tokyo inventory, bounded batch/readiness/result report, production fingerprint, catalog, and runtime artifacts are synchronized; canonical full validation passed and commit/push/deployment are the remaining authorized steps.
- Staged production data is **99 venues / 101 selectable configurations / 154,353 configuration-seat records**. `venues:release:coverage` remains **RELEASE READY: yes**; the fixed Tokyo coverage universe is **40/76 (52.6%)**, MUST **25/44 (56.8%)**, SHOULD **15/28 (53.6%)**. よみうり大手町ホール is the sole Wave15 addressable cohort addition; 南大沢主ホール is outside that fixed universe.
- tokyo-wave3-0041/standard is staged with **501 mapped / 501 official / rangeDiff 0 / representative**. It preserves the complete normal one-floor printed set; the maximum-eight-wheelchair conversion is separate metadata because individual removed IDs are not published.
- tokyo-wave3-0991/standard is staged with **496 mapped / 500 official / rangeDiff -4 / representative**. The four-seat difference is the official, unnumbered wheelchair-space metadata. Printed dotted removable-seat IDs remain in the normal configuration.
...
Normally commit and push the staged TOKYO-WAVE-15 data to `main`, confirm the managed Workers deployment and both public runtime detail endpoints, then record the deployed SHA. Do not research or implement Wave16 without separate authorization.
