...
- **P2 SOL — next recommended, increasing complexity**: run separate bounded Sol high macros for `hbg-hall-release-seed`, then `fukuoka-sunpalace-hall-release-seed`, `festival-hall-release-seed`, and `aichi-arts-center-main-release-seed`. Do not combine them.
...
Run **Sol high bounded evidence-resolution macro for exactly `hbg-hall-release-seed`**: use issuer-owned material to resolve its normal layout, 52-seat pit relationship, and 20 movable wheelchair-seat relationship before range transcription; proceed through two passes and production gate only if one representative numbered-seat set is explicit. Keep `fukuoka-sunpalace-hall-release-seed`, `festival-hall-release-seed`, `aichi-arts-center-main-release-seed`, all P2 holds, and all Kanto/Tokyo records out of scope.
