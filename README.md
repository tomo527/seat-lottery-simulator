...
- [特定商取引法に基づく表記](/tokushoho)（公開環境: https://seat-lottery-simulator.studiotomo.workers.dev/tokushoho）

選択会場のIDは利便性のため、ブラウザのlocalStorage（`seat-lottery-preferences-v1`の`venueId`）へ保存します。現在のアプリケーションコードには、広告、アクセス解析、行動追跡Cookie、問い合わせフォーム、会員登録を実装していません。任意の開発支援は外部のStripe Payment Linkへ遷移するリンクのみで、Stripe SDKや埋め込みCheckoutは使用せず、サイト側でカード番号等の決済情報を取得・保存することはありません（支援額や決済状況などStripeが提供する支払い関連情報は、運営者がStripeの管理画面側で確認できる場合があります。抽選機能とは連携しません）。開示請求先（`src/config/legal.ts`）とPayment Link URL（`src/config/support.ts`）はいずれも設定済みで、支援リンクはTOPページに表示されます。配信にはCloudflare Workers Static Assetsを使用しており、配信・セキュリティ・障害対応のため、Cloudflare側でアクセス情報が処理される場合があります。
